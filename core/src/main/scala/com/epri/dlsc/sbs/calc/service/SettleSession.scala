...
class SettleSession private{
  ...
  private def execute(preparedFormula: Seq[(Formula, Seq[(String, Seq[PreDataSetExpression], String)])]): Boolean = {
    preparedFormula.foreach(formula => {
      ...
      val calcLeftData = if (filters.isEmpty) {
        dataSetInstance.get(formulaInfo.calcDataSetId)
      } else {
        dataSetInstance.get(formulaInfo.calcDataSetId).filter(row => {
          var isTrue = true
      ...
      formula._2.foreach(item => {
        ...
        if (matchedDataSets.size < 1) {
          //(rowid, value)
          newLeftData = executeWithoutDataSet(calcLeftData, calcField, preparedFormulaContent).cache()
        } else {
          //(rowid, value)
          newLeftData = executeWithDataSet(formulaInfo, calcLeftData, matchedDataSets, preparedFormulaContent, scopeCache).cache()
        }
        ...
        val newJoinRdd = dataSetInstance.get(formulaInfo.calcDataSetId).join(valueDF, Array("ROWID"), "left_outer")
      ...
      //需要存储的数据
      val requireSaveData = if (filters.isEmpty) {
        dataSetInstance.get(formulaInfo.calcDataSetId)
      } else {
        dataSetInstance.get(formulaInfo.calcDataSetId).filter(row => {
          var isTrue = true
      ...
      requirePersistDF.add((formulaInfo.calcDataSetId, requireSaveData))
    })
    //数据存储
    requirePersistDF.list().foreach(OracleStorager.save(dataSetConfig, dataSetBroadcastVar, _))
    true
  }
  //不包含数据集的公式计算
  ...
  private def executeWithDataSet(
    ...
    val itemResult = calcParams.mapPartitions(partition => {
      ...
      formulaAlgorithm.put("$", new com.epri.dlsc.sbs.functions.inner.STD())
  ...
  private def normalDataSetHandle(
    ...
    if (cacheData == null) {
      ...
      val leftDataRequireColData = calcLeftData.select(calcLeftData.col("ROWID") :: rightWhere.map(x=>calcLeftData.col(x)).toList: _*)
      val rightDataRequireColData = calcRightData.select(calcRightData.columns.filter(!"ROWID".equals(_)).map(x=>calcRightData.col(x)): _*)
      ...
      //返回
      return joinData.rdd.map(row => (row.getAs[String]("ROWID"), row.getAs[Double](rightCalcField)))
    } else {
      return cacheData
    }
  }
  ...
  /**
    * 计算任务参数配置
    * @param marketId 场景
    * @param sourceTarget 数据源和目标参数
    * @param scriptParams 数据源脚本参数
    * @param formulaIds 公式Id集合
    * @return
    */
  def config(
      marketId: String,
      ...
      formulaIds: Seq[String]): SettleSession
  = {
  ...
  def config(
       ...
       formulaType: String): SettleSession
  ={
    val formulaIds = Configuration.queryFormulaIdsByType(marketId, formulaType, dateTime)
    config = new Config(marketId, sourceTarget, scriptParams, formulaIds)
    this
  }
  ...
  private class RequirePersistDF{
    private val persistDF = mutable.ListBuffer[(String, DataFrame)]()
    def add(value: (String, DataFrame)): this.type = {
      persistDF += value
      this
    }
    def list(): Seq[(String, DataFrame)] = persistDF.toList
  }
  ...
  class ScopeDataCache{
    ...
    def addNormal(key: String, value: DataFrame): Unit = {
      value.cache()
      dataFrameCache += (ScopeDataCache.prefix_NORMAL + key -> value)
    }
    ...
    def addSelf(key: String, value: RDD[(String, Double)]): Unit = {
      value.cache()
      rddDataCache += (ScopeDataCache.prefix_SELF + key -> value)
    }
