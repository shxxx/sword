...
object OracleStorager {
  def save(
    ...
    saveData._2.repartition(3).foreachPartition(partition => {
      ...
      val insertOrUpdateParams = partition.map(row => {
        ...
        var updateIdx = 0
        val fields = dataSetBroadcastConf.fields.filter(field => field.targetColumn != null)
        for(field <- fields if field.targetColumn != null){
  ...
  private def batchExecuteWidthSql(sql: String, params: Seq[Seq[Any]]): Unit = {
