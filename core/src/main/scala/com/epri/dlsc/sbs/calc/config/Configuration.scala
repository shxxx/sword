...
private[calc] object Configuration {
  ...
  def queryDataSets(
    ...
    baseInfoList.map(info => {
      ...
      val executeSql = if(scriptParams != null && !scriptParams.isEmpty){
        replaceSqlParameters(sql, scriptParams.toMap)
      }else{
        sql
      }
  ...
  /**
    ...
    * @return
    */
  def queryFormulaIdsByType(marketId: String, formulaType: String, dateTime: java.util.Date): Seq[String] = {
    val calcDateTime = new SimpleDateFormat("yyyyMMdd").format(dateTime)
    Dao.queryForListWithSql(
      """
        |SELECT ID
        |FROM SE_UPG_FORMULA_DEFINE
        |WHERE MARKET_ID = ?
        |AND FORMULA_TYPE = ?
        |AND TO_DATE(?,'YYYYMMDD') BETWEEN START_TIME AND END_TIME
      """.stripMargin, Array(marketId, formulaType, calcDateTime))
      .map(_.values.head.toString)
  }
